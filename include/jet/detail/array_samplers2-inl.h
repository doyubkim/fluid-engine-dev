...
template <typename T, typename R>
std::function<T(const Vector2<R>&)> NearestArraySampler2<T, R>::functor()
    const {
...
T LinearArraySampler2<T, R>::operator()(const Vector2<R>& x) const {
    ...
    return bilerp(_accessor(i, j), _accessor(ip1, j), _accessor(i, jp1),
                  _accessor(ip1, jp1), fx, fy);
}
...
T CubicArraySampler2<T, R>::operator()(const Vector2<R>& x) const {
    ...
    for (int n = 0; n < 4; ++n) {
        values[n] = monotonicCatmullRom(
            ...
            _accessor(is[2], js[n]), _accessor(is[3], js[n]), fx);
    }
