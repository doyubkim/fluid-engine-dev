...
template <typename T>
T& Array<T, 3>::at(const Size3& pt) {
    return at(pt.x, pt.y, pt.z);
}

template <typename T>
const T& Array<T, 3>::at(const Size3& pt) const {
    return at(pt.x, pt.y, pt.z);
}
