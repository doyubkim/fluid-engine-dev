...
template <typename T>
CudaArray<T, 2>::CudaArray(const Vector2UZ& size, const T& initVal) {
    resize(size, initVal);
}
...
template <typename T>
void CudaArray<T, 2>::resize(const Vector2UZ& size, const T& initVal) {
    _size = size;
...
template <typename T>
const Vector2UZ& CudaArray<T, 2>::size() const {
    return _size;
}
