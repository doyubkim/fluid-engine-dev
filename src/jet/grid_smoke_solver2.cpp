...
double GridSmokeSolver2::smokeDecayFactor() const {
...
void GridSmokeSolver2::computeBuoyancyForce(double timeIntervalInSeconds) {
    ...
    if (std::abs(_buoyancySmokeDensityFactor) > kEpsilonD ||
        ...
        tAmb /= static_cast<double>(
            temp->resolution().x * temp->resolution().y);
...
GridSmokeSolver2::Builder GridSmokeSolver2::builder() {
    return Builder();
}
