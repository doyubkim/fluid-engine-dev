...
void GridFractionalSinglePhasePressureSolver2::buildWeights(
    ...
    _fluidSdf.parallelForEachIndex([&](size_t i, size_t j) {
        _fluidSdf(i, j) = static_cast<float>(fluidSdf.sample(cellPos(i, j)));
    });
    ...
    _system.A.parallelForEachIndex([&](size_t i, size_t j) {
        ...
        if (isInsideSdf(centerPhi)) {
            ...
            if (i > 0) {
                ...
                _system.b(i, j) -= _uWeights(i, j) * input.u(i, j) * invH.x;
            } else {
            ...
            if (j > 0) {
                ...
                _system.b(i, j) -= _vWeights(i, j) * input.v(i, j) * invH.y;
            } else {
            ...
            double boundaryContribution =
                (1.0 - _uWeights(i + 1, j)) * _boundaryVel(uPos(i + 1, j)).x *
                    invH.x -
                (1.0 - _uWeights(i, j)) * _boundaryVel(uPos(i, j)).x * invH.x +
                (1.0 - _vWeights(i, j + 1)) * _boundaryVel(vPos(i, j + 1)).y *
                    invH.y -
