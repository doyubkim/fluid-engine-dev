...
void FaceCenteredGrid2::swap(Grid2* other) {
    FaceCenteredGrid2* sameType
        = dynamic_cast<FaceCenteredGrid2*>(other);
...
double& FaceCenteredGrid2::u(size_t i, size_t j) {
    return _dataU(i, j);
}
...
double& FaceCenteredGrid2::v(size_t i, size_t j) {
    return _dataV(i, j);
}
...
Vector2D FaceCenteredGrid2::valueAtCellCenter(
    size_t i, size_t j) const {
    JET_ASSERT(i < resolution().x && j < resolution().y);

    return 0.5 * Vector2D(
        _dataU(i, j) + _dataU(i + 1, j),
        _dataV(i, j) + _dataV(i, j + 1));
}

double FaceCenteredGrid2::divergenceAtCellCenter(
    size_t i, size_t j) const {
    JET_ASSERT(i < resolution().x && j < resolution().y);
...
Size2 FaceCenteredGrid2::uSize() const {
    return _dataU.size();
}

Size2 FaceCenteredGrid2::vSize() const {
    return _dataV.size();
}
