...
void expectEqual(const T& expected,
                 const MatrixExpression<U, E>& actual) {
    EXPECT_EQ(1, actual.rows());
    EXPECT_EQ(expected, actual.eval(0, 0));
}

template <typename T, typename E>
void expectEqual(const std::vector<T>& expected,
                 const MatrixExpression<T, E>& actual) {
    EXPECT_EQ(expected.size(), actual.rows());

    for (size_t i = 0; i < expected.size(); ++i) {
        EXPECT_EQ(expected[i], actual.eval(i, 0));
    }
}

}  // namespace

TEST(_Vector, Constructors) {
    FloatN<5> t0;
...
TEST(_Vector, Modifier) {
    IntN<5> a{5, 6, 7, 8, 9};

    a.fill(4);
    EXPECT_EQ(IntN<5>(4, 4, 4, 4, 4), a);

    a.fill([](size_t i) -> int { return (int)i; });
    expectEqual({{0, 1, 2, 3, 4}}, a);
}
...
TEST(_Vector, Utilities) {
    ...
    Float3 clampedF3 = clamp(f3, low, high);
