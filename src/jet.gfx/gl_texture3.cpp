...
void GLTexture3::update(const ConstArrayView3<Vector4F>& data) {
    ...
    glTexSubImage3D(glTarget(), 0, 0, 0, 0, static_cast<GLsizei>(_size.x),
                    ...
                    static_cast<GLsizei>(_size.z), GL_RGBA, GL_FLOAT,
                    data.data());
}

void GLTexture3::update(const ConstArrayView3<Vector4B>& data) {
    glBindTexture(GL_TEXTURE_3D, glTextureId());
...
void GLTexture3::onSetTexture(const ConstArrayView3<Vector4F>& data) {
    _size = data.size();
...
void GLTexture3::onSetTexture(const ConstArrayView3<Vector4B>& data) {
    _size = data.size();
